...
import { FacultyController, ModuleController, RoomController, StudentController } from "../../controllers/_Controllers";
...
export default function Dashboard({ user }) {
  ...
  useEffect(() => {
    ...
    let unsubscribeFaculties = FacultyController.subscribeList(snapshot => {
    ...
    let unsubscribeRooms = RoomController.subscribeList(snapshot => {
      ...
      <div className="flex xl:flex-row flex-col justify-between">
        <div className="w-full grid grid-cols-1  md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          <div className="shadow-lg px-4 py-8 flex gap-4 bg-primary rounded-lg text-white">
            <div>
              <svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" preserveAspectRatio="xMidYMid meet" viewBox="0 0 20 20">
            ...
            <div>
              <h2 className="font-semibold uppercase text-lg">Total Faculty :</h2>
              <h4 className="font-bold uppercase text-4xl">{faculties.length}</h4>
            </div>
          </div>
